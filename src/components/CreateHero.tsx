...
import { GameMode, StateArmy } from '../rpg/singlePlayerTroll';
...
export const CreateHero: FC<StateArmy> = ({ state, army }) => {
  const { hero } = state;
  const { publicRuntimeConfig: { version } } = getConfig();

  const handleRollHero = () => army.SET_HERO(mobFactory(
    generateName(),
    rnd(100),
    0,
    uid(),
  ...
  const handleLetsAdventure = () => {
    army.SET_GAME_STATE(GameMode.ADVENTURE_ON_MAP);
    return;
  };
